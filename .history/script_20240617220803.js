...
Ball.prototype.update = function () {
  ...
  this.x += accelerationX(netForceX(dragForceX(data.air, this.dx)), this.mass) * data.timeStep;
  this.y += accelerationY(netForceY(gravitationalForceY(this.mass, data.gravity), buoyantForceY(data.air, this.volume, data.gravity), dragForceY(data.air, this.dy, this.dragCoefficient, this.crossSectionalArea)), this.mass);

  if (this.x + this.radius > canvas.width || thisx - this.radius < 0) {
    this.dx = -this.dx 
  }
  id (this.y + this.radius > canvas.height || this.y - this.radius < 0) {
    this.dy = -this.dy;
  }
...
const balls = [
  new Ball(10, 10, 20, "blue", 5, 2, 20, 0.2)
];
